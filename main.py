...
async def admin_debug(cabinet_id: int, section: str = "ads", st: int = 0, admin: dict = Depends(auth.require_admin), request: Request = None):
    ...
    try:
        if section == "fbs_warehouses":
            ...
            detail = await wb_client.get_finance_detail(token, ninety_ago, today)
            aggregated = await wb_client.get_finance_weekly(token, ninety_ago, today)
            return {
                "date_from": ninety_ago,
                "date_to": today,
                "detail_rows": len(detail),
                "detail_sample_keys": sorted(detail[0].keys()) if detail else [],
                "detail_sample": detail[:2],
